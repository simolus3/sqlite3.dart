      ...
      switch (message) {
        ...
        case _RequestConnection():
          ...
          Future<void> callback(LeasedDatabase db) async {
            ...
            request.client.send(_GrantConnection(
                request.id, db.unsafeRawDatabase.handle.address));
        ...
        case _RequestAllConnections():
          ...
          Future<void> callback(
              List<LeasedDatabase> readers, LeasedDatabase writer) async {
            request.granted = true;

            request.client.send(_GrantAllConnection(
              request.id,
...
final class RemotePool extends PoolImplementation {
  ...
  RemotePool(this._remotePool) {
    Isolate.current.addOnExitListener(_remotePool,
  ...
  Future<T> _withConnection<T>(FutureOr<T> Function(LeasedDatabase db) callback,
      {required bool writer, Future<void>? abort}) async {
    _checkNotClosed();
