...
// A table of ExternalDartReference objects. When we want to call methods on them, we
// pass them as a first argument to a static entrypoint function injected when instantiating
// the module.
static __externref_t host_objects[0];

// Note: Inlining these functions seems to cause linker errors (something about clang
// attempting to take the address of host_objects).
...
static int host_table_grow(int size) {
  return __builtin_wasm_table_grow(host_objects, __builtin_wasm_ref_null_extern(), size);
}

static int host_table_size() {
  return __builtin_wasm_table_size(host_objects);
}
...
typedef struct {
  ...
  // first_free_slot after allocating in that slot. This effectively forms a linked
  // stack of free slots.
  //
...
void* host_object_insert(__externref_t ref) {
  ...
  if (slot == old_capacity) { // Table full?
    ...
    if (target_size < 16) {
      // It's 0 initially, grow it to a reasonable initial size we're unlikely to exceed.
      target_size = 16;
    }
    ...
    size_t* freelist = sqlite3_realloc(host_objects_slab.freelist, target_size * sizeof(size_t));
    for (size_t i = old_capacity; i < target_size; i++) {
      freelist[i] = i + 1;
    }
    // Note: first_free_slot = old_capacity, freelist[old_capacity] = old_capacity + 1 and so on.
