...
void main() {
  ...
  {
    final codegen = TypeScriptCodeGenerator()..generate(types);
    File(
      'extra/js-package/src/generated_protocol.ts',
    ).writeAsStringSync(codegen.buffer.toString());
  }
}
...
abstract base class BaseCodeGenerator {
  final buffer = StringBuffer();

  void generate(List<ProtocolMessageType> types);
}

final class DartCodeGenerator extends BaseCodeGenerator {
  DartCodeGenerator() {
    buffer.write("""
...
final class TypeScriptCodeGenerator extends BaseCodeGenerator {
  TypeScriptCodeGenerator() {
    buffer
      ..writeln(
        "// Generated by tool/protocol_generator.dart, don't modify by hand.",
      )
      ..writeln('import { WebEndpoint } from "./channel.js";');
  }

  String typeNameField(ProtocolMessageType type) {
  ...
  void generate(List<ProtocolMessageType> types) {
    for (final type in types) {
      if (!type.isAbstract) {
        // Put type names in globals to de-duplicate them throughout the
        // package.
        buffer.writeln(
          'export const ${typeNameField(type)} = "${type.messageTypeName}";',
        );
      }
    }
  ...
  void _generateTypes(List<ProtocolMessageType> types) {
    for (final type in types) {
      buffer.write('export ');

      if (type.isAbstract) {
        final subtypes = <String>[];
        for (final subtype in types) {
          if (subtype.inheritsFromName(type.name)) {
            subtypes.add(subtype.name);
          }
        }

        buffer.writeln('type ${type.name} = ${subtypes.join(' | ')};');
        continue;
      }
      ...
      for (final field in type.allFields) {
        buffer
          ..writeln('  // Dart name: ${field.dartName}')
          ..write('  ${field.javaScriptName}: ');
        if (field.isMessageType) {
          buffer.writeln('"${type.messageTypeName}";');
        } else {
          buffer.writeln('${_dartTypeToJs(field.dartType)};');
        }
      }
      buffer.writeln('}');
    }
  ...
  void _generateExtractTransferrable(List<ProtocolMessageType> types) {
    buffer
      ...
      ..writeln('const result: Transferable[] = [];')
      ..writeln('switch (message.t) {');
    ...
    for (final type in types) {
      ...
      final statements = <String>[];
      String addToResult(String expr) {
        return 'result.push($expr);';
      }

      for (final field in type.allFields) {
        final expr = '(message as ${type.name}).${field.javaScriptName}';
        ...
        switch (field.transferMode) {
          ...
          case TransferMode.move:
            ...
            if (field.dartType == 'WebEndpoint') {
              ...
              final tmpName = '${field.dartName}Tmp';
              statements
                ..add('const $tmpName = $expr;')
                ..add('if ($tmpName != null) ${addToResult(tmpName)};');
            } else {
              statements.add(addToResult(expr));
            }
          case TransferMode.moveIfArrayBuffer:
            ...
            statements
              ..add('const $tmpName = $expr;')
              ..add(
                'if ($tmpName instanceof ArrayBuffer) ${addToResult(tmpName)};',
              );
        }
      }
      ...
      if (statements.isNotEmpty) {
        buffer.writeln(
          "case ${typeNameField(type)}: {${statements.join()}break;}",
        );
      }
    }
  ...
  String _dartTypeToJs(String dartTypeName) {
    if (dartTypeName.endsWith('?')) {
      final nonNullable = dartTypeName.substring(0, dartTypeName.length - 1);
      return '${_dartTypeToJs(nonNullable)} | null';
    }
    ...
    return switch (dartTypeName) {
      ...
      'JSArray<JSString?>' => '(string | null)[]',
      'JSArray<JSArray<JSAny?>>' => 'unknown[][]',
      'JSArray' => 'unknown[]',
      _ => 'unknown /* $dartTypeName */',
    };
  }
  ...
  void _generateDispatchMessage(List<ProtocolMessageType> types) {
    ...
    for (final type in typesDirectlyExtendingMessage) {
      if (type != typesDirectlyExtendingMessage.last) {
        ...
        for (final subType in subTypes) {
          buffer.writeln('case ${typeNameField(subType)}:');
        }
      } else {
        // We've covered all other cases, so handle this one as a default.
        buffer.write('default:');
